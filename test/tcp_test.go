...
func TestNewReusablePortListener(t *testing.T) {
	listenerOne, err := reuseport.NewReusablePortListener("tcp4", "localhost:10081")
	if err != nil {
		t.Error(err)
	}
	defer listenerOne.Close()

	listenerTwo, err := reuseport.NewReusablePortListener("tcp", "127.0.0.1:10081")
	if err != nil {
		t.Error(err)
	}
	defer listenerTwo.Close()

	listenerThree, err := reuseport.NewReusablePortListener("tcp6", "[::1]:10081")
	if err != nil {
		t.Error(err)
	}
	defer listenerThree.Close()

	listenerFour, err := reuseport.NewReusablePortListener("tcp6", ":10081")
	if err != nil {
		t.Error(err)
	}
	defer listenerFour.Close()

	listenerFive, err := reuseport.NewReusablePortListener("tcp4", ":10081")
	if err != nil {
		t.Error(err)
	}
	defer listenerFive.Close()

	listenerSix, err := reuseport.NewReusablePortListener("tcp", ":10081")
	if err != nil {
		t.Error(err)
	}
...
func TestNewReusablePortServers(t *testing.T) {
	...
	listenerTwo, err := reuseport.NewReusablePortListener("tcp6", ":10081")
	if err != nil {
		t.Error(err)
	}
	defer listenerTwo.Close()
...
func BenchmarkNewReusablePortListener(b *testing.B) {
	for i := 0; i < b.N; i++ {
		listener, err := reuseport.NewReusablePortListener("tcp", ":10081")
